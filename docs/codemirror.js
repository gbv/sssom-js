...
(function (global, factory) {
    ...
    return function(str, direction) {
      ...
      for (let i$7 = 0; i$7 < len;) {
        if (countsAsLeft.test(types[i$7])) {
          ...
          let pos = i$7, at = order.length, isRTL = direction == "rtl" ? 1 : 0
  ...
  function widgetTopHeight(lineObj) {
    let ref = visualLine(lineObj)
    let widgets = ref.widgets
    let height = 0
    if (widgets) {
  ...
  function wrappedLineExtent(cm, lineObj, preparedMeasure, y) {
    ...
    let begin = findFirst(function (ch) {
  ...
  function coordsBidiPart(cm, lineObj, lineNo, preparedMeasure, order, x, y) {
    ...
    // coordinates.
    let index = findFirst(function (i) {
  ...
  TextMarker.prototype.clear = function () {
    ...
    if (cm && this.collapsed && !cm.options.lineWrapping) {
      for (let i$1 = 0; i$1 < this.lines.length; ++i$1) {
        let visual = visualLine(this.lines[i$1]), len = lineLength(visual)
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    getLine: function(line) {
      let l = this.getLineHandle(line); return l && l.text
    },
  ...
  function addEditorMethods(CodeMirror) {
    ...
    CodeMirror.prototype = {
      ...
      focus: function(){
        win(this).focus(); this.display.input.focus()
      },
