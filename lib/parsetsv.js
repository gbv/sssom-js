...
export const parseTSV = async (input, options = {}) => {
  let { metadata, curie, propagate, metadataHandler, mappingHandler, delimiter } = options

  return new Promise((resolve, reject) => {
    (async () => {
      if (!input) { // empty input
        const sssom = metadata ? await readMetadata(metadata).catch(e => reject(e)) : {}
        if (metadataHandler) {
      ...
      readerOpts.collectMappings = !mappingHandler
      readerOpts.empty = options.empty || !input
      ...
      reader.on("end", sssom => {
        ...
        resolve(sssomOrJskos(sssom, options.to))
      })
    })()
  })
}
