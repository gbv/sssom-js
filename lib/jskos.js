...
const map = {
  ...
  curation_rule: null, // TODO
  curation_rule_text: null, // TODO
...
function mapSlots(sssom, jskos, language) {
  ...
  const contributor = []
  sssom.author_id?.forEach(id => contributor.push(jskosItem(id)))
  sssom.author_label?.forEach(label => contributor.push(jskosItem(null, language, label)))
  if (contributor.length) {
    jskos.contributor = contributor
  }

  const creator = []
  sssom.creator_id?.forEach(id => creator.push(jskosItem(id)))
  sssom.creator_label?.forEach(label => creator.push(jskosItem(null, language, label)))
  if (creator.length) {
    jskos.creator = creator
  }
