...
function mapSlots(sssom, jskos, language) {
  ...
  const creator = sssom.creator || []
  const author = sssom.author || []
  if (creator.length + author.length) {
    jskos.creator = []
    for (let person of [...author, ...creator]) {
      jskos.creator.push( jskosItem(person.id, language, creator.label))
    }
  }
