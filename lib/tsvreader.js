...
export class TSVReader extends EventEmitter {
  ...
  emitError(error, position, value) {
    this.failed = true
    if (typeof error === "string") {
      error = new LocatedError(error, position, value)
    } else {
      error = new LocatedError(error.message, position, value, error)
    }
  ...
  parseEntityReference(value) {
    try {
      ...
      this.emitError(e.message, `line ${this.lineNumber}`)
    }
  }
  ...
  init() {
    ...
    this.tsv.on("headers", headers => {
      ...
      if (missing.length == 1) {
        this.emitError("Missing mapping slot", `line ${this.lineNumber}`, missing[0]) 
      } else if (missing.length > 1) {
        this.emitError("Missing mapping slots", `line ${this.lineNumber}`, missing.join(", ")) 
      }
    })
