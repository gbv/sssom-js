...
import { yamlPosition } from "./locators.js"
...
export class TSVReader extends EventEmitter {
  ...
  emitMetadata(metadata) {
    if (!this.failed) {
      const metadataYaml = this.metadataBlock.join("\n")
      try {
        if (!metadata) {
          metadata = parseMetadata(metadataYaml) || {}
        }
        ...
        const { jsonpointer } = error.position || {}
        if (jsonpointer) {
          const position = yamlPosition(metadataYaml, jsonpointer)
          Object.assign(error.position, position || {})
        }
  ...
  emitMapping(mapping) {
    ...
    for (let [slot,value] of Object.entries(mapping)) {
      if (this.failed) { // stop on first malformed field
        break
      } else if (value !== "") {
    ...
    // remove empty slots
    Object.keys(mapping).filter(slot => !mapping[slot]).forEach(slot => delete mapping[slot])
